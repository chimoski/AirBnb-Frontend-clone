import Image from "next/dist/client/image"
import React, {useState, useRef} from 'react'
import { SearchIcon,GlobeAltIcon, MenuIcon, UserCircleIcon, UsersIcon} from '@heroicons/react/solid'
import 'react-date-range/dist/styles.css'; // main style file
import 'react-date-range/dist/theme/default.css'; // theme css file
import {DateRangePicker} from 'react-date-range'
import {useRouter} from 'next/dist/client/router'
import MobileSearch from "./MobileSearch";
import { useMediaQuery } from 'react-responsive'
import useClickOutside from "../helpers/clickOutside";


export const Header = ({placeholder}) => {
  const smallScreen = useMediaQuery({ query: '(max-width: 768px)' })
  const route = useRouter()
  const [searchInput, setSearchInput] = useState("");
  const [startDate, setStartDate] = useState(new Date());
  const [endDate, setEndtDate] = useState(new Date());
  const [noOfGuests, setNoOfGuests] = useState(1);
  const [openMobileSearch, setOpenMobileSearch] = useState(false);
  const selectionRange ={
    startDate:startDate,
    endDate:endDate,
    key:'selection'
  }
  const searchBar = useRef(null)
  useClickOutside(searchBar,()=>{
    setOpenMobileSearch(false);
   
  })

  const resetInput =()=>{
     setSearchInput("");
  }
  const search =()=>{
    route.push({
      pathname: '/search',
      query: {
        location: searchInput,
        startDate: startDate.toISOString(),
        endDate: endDate.toISOString(),
        noOfGuests
      }
    })
  }
  const handleSelect =(ranges)=>{
    setStartDate(ranges.selection.startDate);
    setEndtDate(ranges.selection.endDate);
  }
  return (
    <>
    <header className="p-5 sticky top-0
     z-50 grid grid-cols-3
    bg-white shadow-md md:px-10
     ">
        {/* left */}
      {
        !openMobileSearch  && (
          <div onClick={()=> route.push("/")}
          className="relative flex
          items-center h-10 cursor-pointer md:hidden
           my-auto">
             <Image
              src="https://links.papareact.com/qd3"
              layout="fill"
               alt="airbnb"
                objectFit="contain"
                objectPosition="left"
              />
         </div>
        )
      }
       <div onClick={()=> route.push("/")}
          className="hidden relative md:flex
          items-center h-10 cursor-pointer 
           my-auto">
             <Image
              src="https://links.papareact.com/qd3"
              layout="fill"
               alt="airbnb"
                objectFit="contain"
                objectPosition="left"
              />
         </div>

        {/* middle */}
        <div className="flex items-center bg-white 
         md:border-2 rounded-full py-2 md:shadow-sm">
            <input className="pl-7 bg-transparent hidden md:inline-flex
             outline-none flex-grow text-sm text-gay-600
            placeholder-gray-400 "
            value={searchInput}
             type="text"
              placeholder={placeholder ||"Start your search"} 
              onChange={(e)=>setSearchInput(e.target.value)}
              />

            <SearchIcon
            onClick={()=>{
              smallScreen &&  setOpenMobileSearch(true);
            }}
             className=" inline-flex h-8 bg-red-400 mx-auto
            text-white rounded-full p-2 cursor-pointer md:mx-2
            "/>
        </div>

      {
        openMobileSearch && smallScreen && (
        
          <div className="flex absolute top-4  mx-[4vw]  w-[90vw] bg-white z-50
          border-2 rounded-full py-2 shadow-sm" ref={searchBar}>
           <MobileSearch
          searchInput={searchInput}
          setSearchInput={setSearchInput}
          placeholder={placeholder}
          SearchIcon ={SearchIcon}
          setOpenMobileSearch={setOpenMobileSearch}
           />
          </div>

        ) 
      }
        
        {/* right */}
      
       
          <div className="flex  items-center space-x-2 justify-end text-gray-500">
          <p className="hidden md:inline cursor-pointer">Become a host</p>
        <GlobeAltIcon className="h-6" />
        <div className="flex items-center space-x-2 border-2 p-2 rounded-full cursor-pointer">
          <MenuIcon className="h-6" />
          <UserCircleIcon className="h-5"  />
        </div>
        </div>
      

        {
          searchInput && (
           <div className="flex flex-col col-span-3 md:mx-auto">
              <DateRangePicker
            ranges={[selectionRange]}
            minDate={startDate}
            rangeColors={['#FD5B61']}
            onChange={handleSelect}
             />

             <div className="flex items-center border-b mb-4">
               <h2 className="text-2xl flex-grow font-semibold">Number of Guests</h2>
              <UsersIcon className="h-5" />
              <input 
              type="number" 
              value={noOfGuests}
              min={1}
              onChange={(e)=>setNoOfGuests(e.target.value)}
              className="w-12 pl-2 text-lg outline-none text-center text-red-400"/>
             </div>
             <div className='flex justify-around'>
              <button onClick={resetInput} className="text-gray-500">Cancel</button>
              <button onClick={search} className="text-red-400">Search</button>
             </div>
           </div>



          )
        }
       
    </header>
    </>
  )
}
